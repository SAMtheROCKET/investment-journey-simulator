## Removed all MATLAB-ish contexts
...
def compute_tax_lotwise(
    ...
    for t, contrib in enumerate(contrib_series):
        ...
        # months held until final redemption
        if sip_timing_start_of_month:
        ...
        gain_lot = fv_lot - contrib  # tax only on gains
...
def add_growth_panel(fig, x_dates, invested_series, fv_series, contrib_series, row=1, col=1):
    ...
    # Monthly SIP bars (shows step-up clearly)
    fig.add_trace(
        go.Bar(
            x=x_dates,
            y=contrib_series,
...
with c_hint:
    st.caption("Edit directly in the table. Presets fill defaults; enable Override to edit tax fields manually.")
...
k3.metric("Total Tax (estimated)", fmt_rupee(portfolio_tax_nom))
k4.metric("Total FV (after-tax)", fmt_rupee(portfolio_fv_after_tax_nom))
...
a2.metric("Real FV (pre-tax)", fmt_rupee(portfolio_fv_final_real))
a3.metric("Real FV (after-tax)", fmt_rupee(portfolio_fv_after_tax_real))

# sip_dashboard_pro.py
# Run:
#   streamlit run sip_dashboard_pro.py
...
def compute_tax_lotwise(
    ...
    for t, contrib in enumerate(contrib_series):
        ...
        gain_lot = fv_lot - contrib  # ✅ tax on gains only
...
# =========================
# Plot helpers: dense grids + stepped stacked area + monthly bars
# =========================
...
# =========================
# Export helpers (PDF snapshot + rich Excel)
# =========================
def fig_to_png_bytes(fig) -> bytes:
    # Requires kaleido installed
    return fig.to_image(format="png", scale=2)

def _write_df_to_sheet(ws, df: pd.DataFrame, start_row=1, start_col=1, header_font=True):
    ...
    for j, col in enumerate(df.columns, start=c0):
        ...
        ws.column_dimensions[chr(64 + (j if j <= 26 else 26))].width = min(max_width, max(10, int(max_len * 0.9)))

def _add_png_to_sheet(ws, png_bytes: bytes, anchor_cell: str, tmpdir: Path) -> None:
    # openpyxl needs a file path (and pillow)
    img_path = tmpdir / f"img_{anchor_cell.replace(':','_').replace('/','_')}.png"
...
def build_excel_bytes_rich(
    *,
    ...
    """
    Rich Excel WITHOUT embedded images (avoids openpyxl image file issues).
      - Dashboard sheet: title + summaries + notes (all visible content text)
      - Funds: fund input table (main page content, not sidebar controls)
      - Nominal_Summary / Real_Summary
      - Nominal_Series / Real_Series
      - Charts_Data: precomputed series ready for charting in Excel
    """
    wb = Workbook()
    ws_dash = wb.active
    ...
    # Charts_Data: make Excel charting easier
    ws_cd = wb.create_sheet(title="Charts_Data")

    # Put both nominal and real series side-by-side
    ...
    _write_df_to_sheet(ws_cd, df_cd, start_row=1, start_col=1)
...
def build_pdf_snapshot_bytes(
    *,
    ...
    """
    PDF snapshot of MAIN PAGE content (excluding sidebar) using in-memory images:
      - Title
      - Nominal summary + nominal chart + nominal table
      - Real summary + real chart + real table
      - Notes/cautions text
    """
    ...
    # ---- Nominal chart (in memory) ----
    nom_png_bytes = fig_to_png_bytes(fig_nom)  # requires kaleido
    nom_img_buf = BytesIO(nom_png_bytes)
    story.append(RLImage(nom_img_buf, width=26*cm, height=16*cm))
    story.append(Spacer(1, 10))
    ...
    # ---- Real chart (in memory) ----
    real_png_bytes = fig_to_png_bytes(fig_real)
    real_img_buf = BytesIO(real_png_bytes)
    story.append(RLImage(real_img_buf, width=26*cm, height=16*cm))
    story.append(Spacer(1, 10))
...
with c_hint:
    st.caption("Edit the table. Presets fill defaults; enable Override to edit tax fields manually.")
...
# -------------------------
# Short portfolio summary (few-liners)
# -------------------------
...
# KPI cards without arrows/deltas
...
title_real = (
    f"Real Returns | Inflation: {inflation_percent:.2f}% | "
...
# =========================
# Notes + usage cautions (restored + expanded)
# =========================
notes_text = (
...
- **Returns are assumptions.** This dashboard assumes a smooth monthly compounding rate derived from your annual % input. Real markets are volatile.
- **Expense ratio handling is an approximation.** Funds deduct expense daily/continuously; we use **net ≈ gross − expense** for planning.
- **Tax shown is a simplification.** Real taxation can include cess/surcharge, fund-category rules, grandfathering, STT, and rule updates over time.
- **Tax is applied on gains only** (not principal) *in this model*, and assumed at final redemption for the whole portfolio.
- **Exit load, TER changes, dividends, rebalancing, switching, and partial withdrawals** are not modeled unless you add those features later.

**How to use efficiently:**
1. Use conservative return assumptions (e.g., index funds 10–12% nominal, debt 6–8% nominal).
2. Enter the fund’s expense ratio from factsheets (TER). Keep inflation realistic (e.g., 4–7%).
3. Use Step-up to model salary growth. If you already plan annual increments, turn Step-up ON.
4. Use staggered start dates only if you truly start different funds at different times.
...
# ---- Build "visible content" text blocks (main page only, exclude sidebar panel itself) ----
dashboard_title = "SIP Portfolio Dashboard"

# These are the same “few-liners” visible in the page
summary_lines_nominal = [
    f"Horizon: {years} years • SIP timing: {sip_timing} • Step-up: {'ON' if stepup_enabled else 'OFF'}" + (f" ({stepup_percent:.2f}%/yr)" if stepup_enabled else ""),
...
# Notes block: include your original sentence + all cautions + usage list (up to point 5)
notes_lines = [
    "Notes: Net return uses net ≈ gross − expense (planning approximation). Equity preset uses STCG 20%, LTCG 12.5%, 12-month threshold, and ₹1.25L exemption on LTCG only. Debt preset (post Apr 1, 2023) is modeled as always STCG at slab rate. This is a simulator for planning; not a tax/financial guarantee.",
    "",
    "Important cautions:",
...
# ---- Funds table (visible) ----
funds_table_df = edited_df.copy()

# ---- Series sheets (already built) ----
...
with c1:
    st.download_button(
        label="⬇️ Download Excel (rich: dashboard + charts + all tables)",
        data=excel_bytes,
...
with c2:
    if pdf_bytes is not None:
        st.download_button(
            label="⬇️ Download PDF (snapshot of main page content)",
            data=pdf_bytes,
